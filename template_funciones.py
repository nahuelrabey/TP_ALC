...
def calculaLU(matriz):
    ...
    for k in range(n - 1):  # recorre columnas de la diagonal principal
        for i in range(k + 1, m):  # filas debajo de la fila k
            if Ac[k, k] == 0:
                print("no se puede continuar")
                return
            factor = Ac[i, k] / Ac[k, k]
            Ac[i, k] = factor  # guardar el factor en lugar del cero

            for j in range(k + 1, n):  # actualizar el resto de la fila
